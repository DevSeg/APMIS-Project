...
public class ProvinceView extends VerticalLayout implements RouterLayout {
	...
	private void export(Grid<RegionIndexDto> grid, TextArea result) {
        // Fetch all data from the grid in the current sorted order
        Stream<RegionIndexDto> persons = null;
//        Set<CampaignFormDataIndexDto> selection = grid.asMultiSelect().getValue();// .asMultiSelect().getValue();
       
            persons = dataView.getItems();
      
        StringWriter output = new StringWriter();
        StatefulBeanToCsv<RegionIndexDto> writer = new StatefulBeanToCsvBuilder<RegionIndexDto>(output).build();
        try {
...
//          boolean matchPosition = String.valueOf(search.getUserPosition()).toLowerCase().contains(searchTerm.toLowerCase());
		...
		Button export = new Button("Reset Filters");
		resetButton.addClassName("resetButton");
		resetButton.addClickListener(e ->{
			dataView.removeFilters();
			regionFilter.clear();
