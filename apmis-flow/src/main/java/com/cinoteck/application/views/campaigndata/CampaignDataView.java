...
public class CampaignDataView extends VerticalLayout {
	...
	private void configureGrid(CampaignFormDataCriteria criteria) {
		...
		Column <CampaignFormDataIndexDto> comm = grid.addColumn(CampaignFormDataIndexDto.COMMUNITY).setHeader("Cluster").setSortable(true).setResizable(true);
...
//	    exporter.setColumnPosition(lastNameCol, 1);
	...
	private void export(Grid<CampaignFormDataIndexDto> grid, TextArea result) {
        ...
        StatefulBeanToCsv<CampaignFormDataIndexDto> writer = new StatefulBeanToCsvBuilder<CampaignFormDataIndexDto>(output).build();
        try {
...
//			newFormButton.setEnabled(true);
		...
		campaignFormCombo.addValueChangeListener(e -> {
			...
			formMetaReference = FacadeProvider.getCampaignFormMetaFacade().getCampaignFormMetaByUuid(e.getValue().getUuid());
		...
		level1Filters.add(campaignFormCombo, regionCombo, provinceCombo, districtCombo, clusterCombo,importanceSwitcher, resetHandler,
				applyHandler);

		HorizontalLayout actionButtonlayout = new HorizontalLayout();
		actionButtonlayout.setVisible(false);
		actionButtonlayout.setAlignItems(Alignment.END);
		actionButtonlayout.add(campaignYear, campaignz, campaignPhase, newForm, popup, importData, exportData);
...
//	            containerPanel.setWidth(containerContent.getWidth(), Unit.PIXELS);
	...
	public void addCustomColumn(String property, String caption) {
		if (!property.toString().contains("readonly")) {
			...
			grid.addColumn(e -> e.getFormValues().stream().filter(v -> v.getId().equals(property)).findFirst().orElse(null)).setHeader(caption).setSortable(true).setResizable(true);
